...
{
	private static Logger LOGGER = LoggerFactory.getLogger(TargetGameApp.class);
...
//        ResourceBundle bundle = ResourceBundle.getBundle(I18n.getBundleBaseName(), I18n.getDefaultLocale());
			...
			Scanner scanner = new Scanner(new FileInputStream("src/main/resources/locale_config.txt"));	// VLAD
    			scanner.useDelimiter("_");																	// VLAD
    			
    			if(scanner.hasNext())																		// VLAD
    				language = scanner.next();																// VLAD
    			
    			if(scanner.hasNext())																		// VLAD
    				country = scanner.next();																// VLAD
    			
    		} catch(IOException e) {																			// VLAD
    			LOGGER.debug("locale_config.txt not found to load");											// VLAD
    		}
    		
        Locale lc = new Locale(language, country);														// VLAD
        
        ResourceBundle bundle = ResourceBundle.getBundle(I18n.getBundleBaseName(), lc);					// VLAD
